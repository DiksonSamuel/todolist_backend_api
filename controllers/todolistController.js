...
const createTodoList = asyncHandler(async (req, res) => {
  ...
  await UserActivity.create({
    user_id: req.user.id.toString(), // MongoDB user ID as a string
    activity: `Created new todolist ${title}`
  });
...
const deleteTodoList = asyncHandler(async (req, res) => {
  ...
  await UserActivity.create({
    user_id: req.user.id.toString(), // MongoDB user ID as a string
    activity: `Deleted todolist - ${todo.title}`
  });
...
const updateTodoList = asyncHandler(async (req, res) => {
  ...
  await UserActivity.create({
    user_id: req.user.id.toString(), // MongoDB user ID as a string
    activity: `Updated todolist - ${todo.title}`
  });
